...
wh_paid <- wh_paid |>
  mutate(
    ...
    sd_salary = sd(SALARY),
    z_score = (SALARY - sd_salary) / mean_salary
  )
...
I would say it is more skewed to the right with a few heavy outliers in the 200,000 range. most of the data lies around the 100,000 and below.
...
It seems a little off with a lower percentage within both 1 and 2 SD by 2.1 and 4.8 pct repectively and the 3 SD being actually at 100% of the data.
...
The normal approximation of the probabilty of a salary of 150,000 dollars is slightly lower than the actual percentage in the real White House salaries.
...
There is a predicted 36% chance of landing in that middle range of the data which is about half of data expected to be within one standard deviation of the mean. The actual number is a bit lower and sits at 32.5%. This shows us that there are less people paid within that range than expected with a normal distribution.
...
A normal distribution predicts a much higher number of salaries to be in the $90k-$120k range while the actual numbers are much lower, in the $60k-$90k range. One glaring reason is because of a much higher number of salaries above $150k. There are 14.1 more than the normal distribution predicts. This will clearly skew the data more than the prediction combined with the greater number of salaries in the lower range. It is much more drawn out than the prediction.
...
Headline: White House Salaries Cluster Outside of Expected Midrange

Lede: Only 32% of White House Salaries fall between $80,000 and $120,000, which is 4% less than expect, according to new data.

The first thing I would compare probabilities to is salaries in different administrations, I would use the normal distribution prediction as a baseline and compare the different administrations to that. We could also analyize more ranges and see how those compare to the expect distribution and the other administrations. 
