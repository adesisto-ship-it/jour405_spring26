...
The variables with the highest relationship to voter turnout seem to be unemployed percentage and white percentage. Other related variables are bachelors and income. The scatterplots shows a relatively strong relationship between turnout are the percentage white and the percentage unemployed, which matches the correlated values. Income does not seem to show a very strong relationship with voter turnout, according to the scatterplot. 
...
Overall, the model seems pretty useful. The adjusted r-squared value is 80%, showing that most of the variables have a high effect on the voter turnout. The p-value, however, is rather high which means that some of these variables might not be super statstically significant. I think that income is one, given its very veyr high estimate. Pct white, however, seems very strong given it's proximity to one. 
...
Somehow the adjusted r-squared values are all rather similar. There is a slight advantage in model 2, which I expected. I thought I was removing some of the variables that I thought were less correlated, but maybe they were playing more of a role than the original scatterplots led me to believe.
...
I could compare how related each variable is to the voter turnout to predict how a given town/county may vote given it's position on these scatterplots. For example, we might be able to predict that given the high percentage of white people in a given town, there might be a higher voter turnout in the next election. This does not account for other potential variables such as who the candidates are or even the approval rating of a given candidate, which might also effect voter turnout.git
