...
This is a relatively useful model that I could use to predict the number of wins given the moderately high adjusted r-squared value.
...
Points scored as the biggest impact on wins. Yards gained seem to have little impact, but there is definitely still some correlation between the two. I would not necessarily use the yards gained to predict wins since the points scored as a much better chance at predicting the wins outcome. 
...
I would probably try to explain how some factors have greater influence on a win outcome vs others. I could use, for example, the number of yards gained be each team and the points scored to predict whether one might win over another since they both have somewhat of an effect on the chance of a win.
